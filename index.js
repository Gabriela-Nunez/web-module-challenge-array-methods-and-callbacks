...
//(a) Home Team name for 2014 world cup final
 fifaData.filter((name) => {
    if(name.Year === 2014) {
    console.log('Task 1a:', name["Home Team Name"]);
    }
})
...
fifaData.filter((name) => {
if(name.Year === 2014) {
    console.log('Task 1b:', name["Away Team Name"]);
}
})
//(c) Home Team goals for 2014 world cup final
fifaData.filter((goals) => {
    if(goals.Year === 2014) {
        console.log('Task 1c:', goals["Home Team Goals"]);
    }
    })
//(d) Away Team goals for 2014 world cup final
fifaData.filter((goals) => {
    if(goals.Year === 2014) {
        console.log('Task 1d:', goals["Away Team Goals"]);
    }
    })
//(e) Winner of 2014 world cup final */
fifaData.filter((winner) => {
    if(winner.Year === 2014 && winner.Stage === "Final") {
        console.log('Task 1e:', winner["Win conditions"]);
    }
    })
...
function getWinnersByYear(array, cb2, cb3, cb4) {
    ...
    return cb3(array, cb2).map((element, index) => `In ${element}, ${winners[index]} won the world cup!`);
}
...
function getAverageGoals(cb2) {
    const totalGoals = cb2.reduce((prev, current) => {
    return prev + current["Home Team Goals"] + current["Away Team Goals"];
    }, 0);
    return (totalGoals / cb2.length).toFixed(2);
 }
